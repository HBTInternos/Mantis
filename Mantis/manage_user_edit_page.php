...
<tr class="row-1">
	<td class="category">
		<?php echo lang_get( 'autentication' ) ?>
	</td>
	<td>
		<select name="autentication" >
		<?php print_enum_string_option_list( 'login_methods', $t_user['acceso']) ?>
			<!-- <option value = "ldap">Autenticación LDAP</option>
			<option value = "mantis">Autenticación Mantis</option> -->
		</select>
	</td>
</tr>
<tr><?php  //Fin ADD HBT ?>
...
<?php if( $t_impersonate ) { ?>
	<form id="manage-user-impersonate-form" method="post" action="manage_user_impersonate.php" class="pull-left">
		<fieldset>
			...
			<input type="hidden" name="user_id" value="<?php echo $t_user['id'] ?>" />
			<!-- se comenta HBT <input type="submit" class="btn btn-primary btn-white btn-round" value="<?php echo lang_get( 'impersonate_user_button' ) ?>" /> -->
		</fieldset>
	</form>
<?php } ?>
