...
require_api( 'projax_api.php' ); // ADD HBT 
...
if( $f_master_bug_id > 0 ) {
	...
	$f_impact_analysis		= $t_bug->impact_analysis; // ADD HBT 
	...
	$f_impact_analysis		= gpc_get_string( 'impact_analysis', '' );
...
//$t_show_reproducibility = in_array( 'reproducibility', $t_fields ) ; // original HBT
$t_show_reproducibility = in_array( 'reproducibility', $t_fields ) && $control_project != 'control de'; // Modificado HBT
$t_show_eta = in_array( 'eta', $t_fields );
$t_show_severity = in_array( 'severity', $t_fields );
$t_show_priority = in_array( 'priority', $t_fields );
//$t_show_steps_to_reproduce = in_array( 'steps_to_reproduce', $t_fields );// original HBT
$t_show_steps_to_reproduce = in_array( 'steps_to_reproduce', $t_fields )&& $control_project != 'control de';// Modificado HBT
$tpl_show_impact_analysis = $control_project == 'control de'; // ADD HBT
...
//$t_show_profiles = config_get( 'enable_profiles' );// original HBT
$t_show_profiles = config_get( 'enable_profiles' )&& $control_project != 'control de';// Modificado HBT
...
//$t_show_additional_info = in_array( 'additional_info', $t_fields ); // original HBT
$t_show_additional_info = in_array( 'additional_info', $t_fields )&& $control_project != 'control de'; // Modificado HBT
...
<?php if( $t_show_platform || $t_show_os || $t_show_os_build ) { ?>
	<!-- se comenta HBT requerimeinto 24/04/2023 migracion al servidor en la nube<tr>
		<th class="category">
...
<?php } ?>
<!-- ADD HBT -->
<?php if ( $tpl_show_impact_analysis ) { ?>
		<tr <?php echo helper_alternate_class() ?>> 
			<td class="category">
				<?php print_documentation_link( 'impact_analysis' ) ?>
			</td>
			<td>
				<textarea <?php echo helper_get_tab_index() ?> name="impact_analysis" cols="80" rows="10"><?php echo string_textarea( $f_impact_analysis ) ?></textarea>
			</td>
		</tr>
<?php }

#Rqto_977 - ayoung
#Obtenemos el o los IDs del o los proyectos que apliquen este nuevo campo al reportar entrega
$error_reopening_projects_ids = config_get('error_reopening_projects');
$array_ids = explode(',', $error_reopening_projects_ids);
#Recorremos los proyectos, y validamos si aplica para el actual
if (in_array($t_project_id, $array_ids)){
	?>
	<tr>
		<!-- Campo 'Reapertura Error' -->
		<td class="category"><?php echo lang_get('error_reopening_label'); ?></td>
		<td style="background: #DBDBDB;"><select 
		<?php echo helper_get_tab_index() ?> name="cbx_error_reopening"
			onchange="habilitar(this)" id="reopen">
			<option value="0"><?php echo 'NO';?></option>
			<option value="1"><?php echo 'SI'; ?></option>
		</select></td>
	</tr>
	<?php #En este punto se crea el campo para ingresar el texto de la incidencia, al haberse seleccionado 'SI'
	#en el par�metro 'Reapertura Error'?>
	<tr>
		<!-- Campo 'Ingresar Incidencia(s)' -->
		<td class="category"><?php echo lang_get('reopining_text_label'); ?></td>
		<!-- Se crea el campo texto disabled, ya que con el script habilitar() que est� en el onchange de cbx_error_reopening
		se habilita al seleccionar 'SI' en dicho combobox-->
		<td style="background: #DBDBDB;"><input type="text" name="incidences"
			disabled="disabled" onkeypress="return validarn(event)" /></td>
	</tr>
	<?php
}?>
<!-- FIN HBT -->
