...
// add HBT
$tpl_show_impact_analysis = in_array( 'impact_analysis', $t_fields );
$tpl_impact_analysis_textarea = $tpl_show_impact_analysis ? string_textarea( $t_bug->impact_analysis ) : '';
// fin add HBT
...
// add HBT
	$control_project = $t_bug->project_id;
...
//Se modifica para validar si el campo es un control de cambios HBT
if ($control_project != 'control de'){ // ADD HBT
...
# Steps to Reproduce
if ($control_project != 'control de'){ // ADD HBT se agrega el if 
...
// ADD HBT
#  impact_analysis todo este bloque es perzonalizado
if ($control_project == 'control de'){
	if ( $tpl_show_impact_analysis ) {
		echo '<tr >';
		echo '<td class="category">', lang_get( 'impact_analysis' ), '</td>';
		echo '<td colspan="5">';
		echo '<textarea ', helper_get_tab_index(), ' cols="80" rows="10" name="impact_analysis">', $tpl_impact_analysis_textarea, '</textarea>';
		echo '</td></tr>';
	}
} // FIN ADD HBT

	#Rqto_977 - ayoung HBT
#En este punto anexaremos la informaci�n del campo 'Reapertura Error', as� podr� ser modificable
...
if (in_array($t_bug->project_id, $array_ids)){
	#Obtenemos las incidencias asociadas
	$associated_incidences = get_reopened_bug_to_bug($tpl_bug_id);
	#Validamos lo que se mostrar�, en caso de ser null se mostrar� 'No'
	if ($associated_incidences == NULL){
		$associated_incidences = "";
		$disabled = 'disabled';		
	} else {
		#De lo contrario tiene incidencias asociadas y necesitamos por defecto el valor 'SI' en la 
		#lista de cbx_error_reopening
		$selected = 'selected';
	}
	#Mostramos la info asociada
	echo '<tr >';
	#label 'Reapertura Error'
	echo '<td class="category">', lang_get( 'error_reopening_label' ), '</td>';
	#lista SI/NO Reapertura Error
	echo '<td class="none" colspan="5">';
	echo '<select ', helper_get_tab_index(), ' name="cbx_error_reopening" onchange="habilitar(this), inform(this)">';	
	...
	echo '</td>';
	echo '</tr>';
	#fila de las incidencias asociadas
	echo '<tr >';
	echo '<td class="category">', lang_get( 'reopining_text_label' ), '</td>';
	echo '</td>';
	if ($disabled == NULL){
		echo '<td colspan="5">', '<input ', helper_get_tab_index(), ' type="text"  name="incidences" value="'.$associated_incidences.'"
			onkeypress="return validarn(event)" />';
	} else{
		echo '<td colspan="5">', '<input ', helper_get_tab_index(), ' type="text"  name="incidences" value="'.$associated_incidences.'"
			disabled="disabled" onkeypress="return validarn(event)" />';	
	}	
	echo '</tr>';
}
#Fin Rqto_977 - ayoung HBT
...
foreach ( $t_related_custom_field_ids as $t_id ) {
	...
	//Inicio HBT
	//Se muestra el campo personalizado etapa de detección sin permiso de escritura
	if($t_def['id']==1)
	{
...
//Imprime en pantalla con acceso a escritura los campos personalizados HBT
		//para la vista menos el campo etapa de deteccion HBT
		if($t_def['id']!=1){ // ADD HBT
			echo '<tr>';
			echo '<td class="category">';
