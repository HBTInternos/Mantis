...
<?php
	...
	$t_bug_is_open = in_array( $t_current_resolution, array( config_get( 'default_bug_resolution' ), config_get( 'bug_reopen_resolution' ) ) ); // ADD HBT 
...
<!-- Resolution -->
			...
			<?php
			$t_resolution = $t_bug_is_open ? config_get( 'bug_resolution_fixed_threshold' ) : $t_current_resolution; // ADD HBT
			
			print_enum_string_option_list( "resolution" ); #HBT Se elimina el segundo parametro para evitar que otro item se muestre por defecto
				/* HBT SE comenta codigo original
				...
				print_enum_string_option_list( 'resolution', $t_resolution ); fin HBT*/
			?>
					</select>
				</td>
			</tr>
...
?>
	<tr>
		<th class="category">
			<!-- HBT codigo original <?php if( $t_require && $t_has_write_access ) {?><span class="required">*</span><?php } ?> -->
			<!-- HBT Modificacion --> <?php if( $t_require && $t_has_write_access && $t_def['id']!=1) {?><span class="required">*</span><?php } ?>
...
<?php
			if( $t_has_write_access ) {
				...
				if ( custom_field_has_read_access( $t_id, $f_bug_id ) ) {
					?>	
					<?php echo print_custom_field_value( $t_def, $t_id, $f_bug_id );	?>
