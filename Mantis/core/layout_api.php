...
function layout_navbar() {
	...
	// ADD HBT
	$t_logo_heinsohn = config_get_global('logo_heinsohn');
	// FIN ADD HBT

	// echo '<div id="navbar"  class="navbar navbar-default navbar-collapse navbar-fixed-top noprint">';
	// Modificacion HBT
	echo '<div id="navbar"  style="background: white;" class="navbar navbar-default navbar-collapse navbar-fixed-top noprint">';
	// fin Modificacion 
	...
	echo '<div class="navbar-header" style="width:70%;">';
	echo '<a href="' . $t_short_path . $t_logo_url . '" class="navbar-brand">';
	echo '<span class="smaller-75"> ';
	// echo string_display_line( config_get('window_title') ); Modificacion HBT
	echo 	'<img style="width.117%" src="'.$t_short_path . $t_logo_heinsohn .'"></img>'; 
	// FIN Modificacion 
	echo ' </span>';
	echo '</a>';
	...
	echo '<div style="margin-top: 33px;" class="navbar-buttons navbar-header navbar-collapse collapse">';
	// echo '<div  class="navbar-buttons navbar-header navbar-collapse collapse">'; Modificacion HBT
	echo '<ul class="nav ace-nav">';
	if (auth_is_user_authenticated()) {
		# shortcuts button bar
		// layout_navbar_button_bar(); Modificacion HBT
...
function layout_main_content_begin() {
	echo '<div class="main-content" style="  border: 2px solid #e8e8e8;
    border-radius: 12px;
    padding: 5px;">' , "\n";
}
...
function layout_breadcrumbs() {
	...
	// Modificado por HBT
	echo '<div id="breadcrumbs" style="background:#cfcfcf6e" class="breadcrumbs noprint">' , "\n";
	// FIN Modificacion HBT
	# Login information
...
function layout_footer() {
	...
	if( !is_page_name( 'login_page' ) ) {
		...
		echo '<a href="https://www.mantisbt.org" '.
			'title="Mantis Bug Tracker: Sistema de seguimiento de errores, free and open source.">' .
			'<img src="' . $t_mantisbt_logo_url .  '" width="102" height="35" ' .
			'alt="Mantis Bug Tracker: Sistema de seguimiento de errores, free and open source." />' .
			'</a>' . "\n";
		echo '</div>' . "\n";
		echo '</div>' . "\n";
	}
...
function layout_login_page_logo() {
	...
	<div class="login-logo">
		<img style="width: 132%;margin-left: -100px;" src="<?php echo helper_mantis_url( config_get_global( 'logo_image' ) ); ?>">
		<!-- <img src="<?php echo helper_mantis_url( config_get_global( 'logo_image' ) ); ?>"> Modificacion HBT -->
	</div>
	<?php
}
