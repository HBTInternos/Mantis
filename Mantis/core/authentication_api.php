...
function auth_auto_create_user( $p_username, $p_password ) {
	// ADD HBT
	// $t_login_method = config_get_global( 'login_method' ); se comenta

	$t_user_access = user_get_acceso_by_name ( $p_username ); /* buscar que tipo de acceso tiene el usuario */
...
function auth_attempt_login( $p_username, $p_password, $p_perm_login = false ) {
	...
	// ADD HBT
	$t_user_access = user_get_acceso_by_name ( $p_username ); /* buscamos el tipo de acceso que tiene el usuario */ 
...
function auth_does_password_match( $p_user_id, $p_test_password ) {
	...
	$t_acceso = user_get_acceso_by_id( $p_user_id ); /* buscamos el tipo de acceso del usuario */
	...
	foreach( $t_login_methods as $t_login_method ) {
		...
		// ADD HBT 
		// if( auth_process_plain_password( $p_test_password, $t_password, $t_login_method ) == $t_password ) { codigo original
		if( auth_process_plain_password( $p_test_password, $t_password,$p_user_id, $t_login_method ) == $t_password ) { /* HBT Se agrega el parametro $p_user_id */
 ...
 */
// ADD HBT 
function auth_process_plain_password( $p_password,$p_id = null, $p_salt = null, $p_method = null ) { /* HBT se agrega el parametro $p_id = null */
// function auth_process_plain_password( $p_password, $p_salt = null, $p_method = null ) { Codigo original HBT 
	// $t_login_method = config_get_global( 'login_method' ); se comenta HBT 

	$t_acceso = user_get_acceso_by_id($p_id);// Buscamos el tipo de acceso del usuario HBT
	if($t_acceso != 2){
