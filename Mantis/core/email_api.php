...
function email_signup( $p_user_id, $p_confirm_hash, $p_admin_name = '' ) {
	...
	$t_create_account = 1;//ADD HBT
	// Bloque HBT Validación para distinguir que correo se debe enviar 
	// Dependiento el acceso del usuario 
	$t_acceso = user_get_acceso_by_name($t_username); // ADD HBT 
	// el siguiente if es agregado por HBT 
	// acceso =1 es cuando el usuario es de HBT y se conecta por LDAP
	if($t_acceso==1){
		$t_subject = '[' . config_get( 'window_title' ) . '] ' . lang_get( 'new_account_subject' );
		$t_intro_text = sprintf( lang_get( 'new_account_HBT_User' ), $t_username ); 
		$t_message = $t_intro_text . "\n\n" .string_get_confirm_hash_url_HBT(). "\n\n" . lang_get( 'new_account_message' ) . "\n\n" . lang_get( 'new_account_do_not_reply' );
	}else{
	...
	if( !is_blank( $t_email ) ) {
		//email_store( $t_email, $t_subject, $t_message, null, true ); Se comenta HBT
		log_event( LOG_EMAIL, 'Signup Email = %s, Hash = %s, User = @U%d', $t_email, $p_confirm_hash, $p_user_id );
		// ADD HBT
		$t_email_data = email_data($t_email, $t_subject, $t_message);
		$resultado = email_send($t_email_data, $t_email, $t_message,$t_username, $t_create_account);
		return $resultado;
		//FIN ADD HBT
	}
...
// ADD HBT
function user_get_username1( $p_user_id ) {
	...
	$t_result = db_query( $query, array( $p_user_id ) );
...
function email_send_confirm_hash_url( $p_user_id, $p_confirm_hash, $p_reset_by_admin = false ) {
	...
	# retrieve the username and email
	$t_username = user_get_username1( $p_user_id ); // HBT SE cambia por el metodo user_get_username1 original user_get_username
	$t_email = user_get_email( $p_user_id );
	...
	if( !is_blank( $t_email ) ) {
		...
		if( OFF == config_get( 'email_send_using_cronjob' ) ) {
			...
			$reset_password = 2;
			$resultado = email_send($t_email_data);
			
		} // FIN ADD HBT

	} else {

		$p_email_blank = true; // ADD HBT 
		//log_event( LOG_EMAIL, 'Password reset for user @U%d not sent, email is empty', $p_user_id ); SE comenta HBT 
	}
...
function email_generic_to_recipients( $p_bug_id, $p_notify_type, array $p_recipients, $p_message_id = null, array $p_header_optional_params = null ) {
	...
	$t_ok = true;
	$estado;
	// if( empty( $p_recipients ) ) { Se comenta HBT
		db_param_push();
	...
	if( $t_row ) {
		...
		$reportador = $t_row['reporter_id'];
		$asignado = $t_row['handler_id'];
	
		if($estado==80 || $estado==90){
			$t_query1 = 'SELECT id, email FROM {user} 
					  WHERE  id=' . db_param() . ' OR id=' . db_param();
	
			$t_result1 = db_query( $t_query1, array( $reportador,$asignado ));
			$t_row1 = db_fetch_array( $t_result1 );
			if($t_row1){
				
				// print_r($t_row1);
				$p_recipients[$t_row1['id']]=$t_row1['email']; 
				// print_r($p_recipients);
			}
		}
	}
	// } FIN ADD HBT 
	...
	if( is_array( $p_recipients ) ) {
		...
		foreach( $p_recipients as $t_user_id => $t_user_email ) {
			...
			// ADD HBT 
		//	email_bug_info_to_one_user( $t_visible_bug_data, $p_message_id, $t_user_id, $p_header_optional_params ); Se comenta HBT 
			$t_ok =email_bug_info_to_one_user( $t_visible_bug_data, $p_message_id, $t_project_id, $t_user_id, $p_header_optional_params )&& $t_ok; // Modificacion HBT
			if($t_ok == false)
			$ban = 1;
			// FIN HBT 
			lang_pop();
...
function email_monitor_added( $p_bug_id, $p_user_id ) {
	...
	// ADD HBT 
	// email_generic( $p_bug_id, 'monitor', 'email_notification_title_for_action_monitor', $t_opt, array( $p_user_id ) ); origianl 
	$resultado=email_generic( $p_bug_id, 'monitor', 'email_notification_title_for_action_monitor', $t_opt, array( $p_user_id ) ); // Modificado HBT 
	return $resultado; //FIn add HBT 
}
...
function email_new_bug( $p_bug_id ) {
	$resultado = email_generic( $p_bug_id, 'new', 'email_notification_title_for_action_bug_submitted' );
	return $resultado;
}
// FIN ADD HBT 
...
function email_resolved( $p_bug_id ) {
	...
	// ADD HBT 
	// email_generic( $p_bug_id, 'resolved', 'email_notification_title_for_status_bug_resolved' ); orignal
	$resultado=email_generic( $p_bug_id, 'resolved', 'email_notification_title_for_status_bug_resolved' ); /* modificado */
	return $resultado;
	// FIN ADD HBT
}
...
function email_assign( $p_bug_id ) {
	$resultado = email_generic( $p_bug_id, 'owner', 'email_notification_title_for_action_bug_assigned' );
	return $resultado;
}
...
function email_reopen( $p_bug_id ) {
	$resultado = email_generic( $p_bug_id, 'reopened', 'email_notification_title_for_action_bug_reopened' );
	return $resultado;
}
// FIN ADD HBT
...
function email_close( $p_bug_id ) {
	...
	// ADD HBT
	// $resultado=email_generic( $p_bug_id, 'closed', 'email_notification_title_for_status_bug_closed' ); Original
	$resultado=email_generic( $p_bug_id, 'closed', 'email_notification_title_for_status_bug_closed' ); /* Modificado HBT */
	return $resultado;
	// fin HBT
}
...
function email_bug_deleted( $p_bug_id ) {
	...
	// ADD HBT
	// email_generic( $p_bug_id, 'deleted', 'email_notification_title_for_action_bug_deleted' ); original 
	$resultado=email_generic( $p_bug_id, 'deleted', 'email_notification_title_for_action_bug_deleted' ); /* modificado HBT */
	return resultado;
	// fin HBT
}

/**
 * Esta funcion para obtener los datos del correo HBT
 *
 */
function email_data($p_recipient, $p_subject, $p_message,$p_headers = null){
	...
	foreach( $t_ids as $t_id ) {
		...
		if( !$t_email_sent ) {
			...
			if( $p_delete_on_failure || $t_submitted < $t_retry_cutoff ) {
				...
				//email_queue_delete( $t_email_data->email_id, $t_reason ); Comentado HBT 
			}
 ...
 * @return void
 */
// function email_bug_info_to_one_user( array $p_visible_bug_data, $p_message_id,  $p_user_id, array $p_header_optional_params = null ) { origianl HBT
function email_bug_info_to_one_user( array $p_visible_bug_data, $p_message_id, $p_project_id, $p_user_id, array $p_header_optional_params = null ) { /* modificado HBT se agrega el parametro  $p_project_id*/
	...
	# send mail HBT Se comenta no se guarda en BD el log del envio del correo
	// email_store( $t_user_email, $t_subject, $t_message, $t_mail_headers );

	$t_email_data = email_data($t_user_email, $t_subject, $t_message);
	$result= user_get_name( $p_user_id );
	if ( OFF == config_get( 'email_send_using_cronjob' ) ) {
		$resultado = email_send($t_email_data, $t_user_email, $t_message,$result, $t_mail_headers, $t_bug_id);
