...
function custom_field_get_value( $p_field_id, $p_bug_id ) {
	...
	$t_custom_field_string_table = 'mantis_custom_field_string_table' ;
	$query = "SELECT value
...
function number_validate_requirement( $p_val ) {		
	$tam = strlen($p_val);
	...
	if ($tam == 0){
		return false;
			...
			$letra = substr($p_val, 0, 1);
			$letra2 = substr($p_val, 1, 1);			
			
			if (strcmp($letra, "R") != 0 && strcmp($letra, "A") != 0){				 
				return false;
			}
			if(strcmp($letra2, "-") != 0){
				return false;	
			...
			for ($i=2; $i<$tam; $i++){
				$letra3 = substr($p_val, $i, 1);
				if (!strpos($permitidos, $letra3)){				 
					return false;
				}
			}
			return true;
