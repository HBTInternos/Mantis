...
function user_create( $p_username, $p_password, $p_email = '',
	...
	db_param_push();
	// ADD HBT se modifica la inserción para insertar el campo acceso
	$t_query = 'INSERT INTO {user}
				    ( username, email, password, date_created, last_visit,
				     enabled, access_level, login_count, cookie_string, realname,acceso )
				  VALUES
				    ( ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param()  . ',
				     ' . db_param() . ',' . db_param() . ',' . db_param() . ',' . db_param() . ', ' . db_param() . ', ' . db_param() . ')';
	db_query( $t_query, array( $p_username, $p_email, $t_password, db_now(), db_now(), $c_enabled, (int)$p_access_level, 0, $t_cookie_string, $p_realname,(int)$p_acceso ) );
...
function user_reset_password( $p_user_id, $p_send_email = true ) {
	...
	if( ( ON == config_get( 'send_reset_password' ) ) && ( ON == config_get( 'enable_email_notification' ) ) ) {
		...
		if( $p_send_email ) {
			...
			$resultado = email_send_confirm_hash_url( $p_user_id, $t_confirm_hash, true ); // Modificado HBT 
			if($resultado == false) //HBT
				return null; //HBT
		}
	} else {
...
// ADD HBT
function user_get_acceso_by_id( $p_id ) {
...
//Inicio HBT
function user_get_acceso_by_name( $p_username ) {
	...
	$t_result = db_query( $query, array( $p_username ) );
	$t_row = db_fetch_array( $t_result );
	if( $t_row ) {
		
		return (int)$t_row['acceso'];
	}
}

// fin HBT
//Inicio HBT
function user_get_acceso_by_login_count( $p_username ) {
	
	db_param_push();
	$query = 'SELECT login_count
				  FROM mantis_user_table
				  WHERE username=' . db_param();
	...
	if( $t_row ) {
		...
		return (int)$t_row['login_count'];
	}
}


function user_validar_eliminacion_HBT( $user_id ) {
	
	db_param_push();
	$query = 'SELECT * FROM mantis.mantis_bug_table b 
	INNER JOIN mantis.mantis_bugnote_table n ON b.id=n.bug_id
	WHERE b.reporter_id=' . db_param().' OR b.handler_id=' . db_param().' OR n.reporter_id=' . db_param();
	
	$t_result = db_query( $query, array( $user_id,$user_id,$user_id) );
