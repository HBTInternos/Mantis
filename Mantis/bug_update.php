...
$t_updated_bug->impact_analysis = gpc_get_string( 'impact_analysis', $t_existing_bug->impact_analysis ); // add HBT
...
// ADD HBT
	$control_project = $t_updated_bug->project_id;
	$control_project = consultar_project($control_project);	
	$control_project = strtolower($control_project);
	$control_project = substr($control_project, 0, 10);
// FIn HBT
...
#HBT 
$t_old_bug_resolution=$t_updated_bug->resolution;

#Requerimiento 865
if ($t_updated_bug->project_id == config_get('global_project_proteccion')) {
	$t_updated_bug->handler_hbt_id			= gpc_get_int( 'handler_hbt_id', $t_updated_bug->handler_hbt_id );
}

# Inicio Rqto 679 basado en Req 671. No se permite que la resolucion sea resuelta o cerrada como abierta o reabierta
if ((($t_updated_bug->resolution == 10 || $t_updated_bug->resolution == 30))&&(($t_updated_bug->status == 80)||($t_updated_bug->status == 90))){
	trigger_error( ERROR_RESOLUTION_RESOLVED_CLOSED, ERROR ); 
}
# Fin HBT. No se permite que la resolucion sea resuelta o cerrada como abierta o reabierta
...
// HBT Modificacion 
if( $t_updated_bug->status ==80
) {
	$t_resolve_issue = true;
} else if( $t_updated_bug->status ==90
) { /* FIN HBT */ 
	$t_close_issue = true;
...
# HBT basado en Req 671. No se permite que la resolucion se quede sin informacion.
if ($t_updated_bug->resolution == 0){
	lang_get( 'select_option' );
	error_parameters( lang_get_defaulted( 'Resolucion' ) );
	trigger_error( ERROR_EMPTY_FIELD, ERROR );
} // Fin HBT
...
foreach ( $t_related_custom_field_ids as $t_cf_id ) {
	...
	// ADD HBT
	//Ac� estaba la validaci�n si un campo personalizado requerido est� vac�o, se movi�
	//Por requerimiento 679.
	
	if(in_array(1,$t_related_custom_field_ids))	{
		
		$s_bug_id = gpc_get_int( 'bug_id' );
		$value_detection= custom_field_get_value( 1, $s_bug_id  );
	}
	if($t_cf_def['id']==2){
	
		$value_inyection = gpc_get_custom_field( "custom_field_$t_cf_id", $t_cf_def['type'], $t_cf_def['default_value'] );
	}

	// 679  Se valida que el campo personalizado requerimiento padre Vernue
	// No tenga ceros a la izquierda o no este vacio, mirando que estado es el que 
	//se quiere actualizar en este caso 80 Resuelto y 90 Cerrado
	if ($t_cf_def['id']==4){
		if($t_updated_bug->status == 80 ||$t_updated_bug->status == 90){
			$value = gpc_get_custom_field( "custom_field_$t_cf_id", $t_cf_def['type'], $t_cf_def['default_value'] );			
			
			if ((!number_validate_requirement($value))){
				//Reqto 805
				//Mientras se este trabajando con este proyecto el mensaje de Error debe salir con 'Requerimiento Vernue'.
				//if($t_bug_data->project_id == config_get('global_project_archivo')&& $t_def['id'] == config_get('id_requerimiento_Vernue')){
					//error_parameters( lang_get('requerimiento_vernue'));
				//} else {
					// Fin Reqto 805
					error_parameters( lang_get_defaulted( custom_field_get_field( $t_cf_id, 'name' ) ) );
					//error_parameters( lang_get('requerimiento_vernue'));
				//}
				trigger_error( ERROR_CUSTOM_FIELD_INVALID_REQUERIMIENTO, ERROR );
			}
		}
	}//fin 679
	// Se valida que la Etapa de Inyecci�n no sea superior a la Etapa de Detecci�n
	// Rqto 1003 ltaborda
	//Se valida si el proyecto es Control de Cambios en cuyo caso no se valida la etapa de inyecci�n
	
	if ($control_project != config_get('proyecto_control_de_cambios'))	{
		if ($value_inyection > $value_detection){
			echo 'inyección: '.$value_inyection;
			echo '<br></br>';
			echo 'detección: '.$value_detection;
		trigger_error( ERROR_STEP_VALIDATION, ERROR );
		}
	
	// Fin Rqto 679 (683)
	}
	
	// Fin Rqto 1003 ltaborda
		...
		#Requerimiento 809
	#Sentencia realizada para asignar un valor a las variables $t_new_custom_field_value y $t_custom_field_value
	#para que permita la edici�n del campo personalizado "Incidencia Reabierta".

	if ($t_old_bug_resolution != config_get('global_id_resolution')){
		if ($t_cf_def['id']== config_get('global_id_custom')&& $t_updated_bug->resolution== config_get('global_id_resolution')){
			$t_cf_def['default_value']= lang_get ('Estado_incidencia_reabierta');
			$t_custom_field_value= $t_cf_def['default_value'];
			$t_new_custom_field_value= $t_cf_def['default_value'];
		}else {
			$t_cf_def['default_value']= lang_get('Estado_incidencia_Noreabierta');
		}
	}
	#Fin Requerimiento 809
	$t_custom_field_value = gpc_get_custom_field( "custom_field_$t_cf_id", $t_cf_def['type'], null );// 679
	//Sentencia if realizada para que el campo el campo etapa de detecci�n
	//que no puede editarse no se actualice con valor en blanco Req 679 (683)
	//Se movi� por requerimiento 679
	// FIN HBT
...
// HBT Se agrega la columna 
// $t_existing_bug->impact_analysis != $t_updated_bug->impact_analysis);
$t_text_field_update_required = ( $t_existing_bug->description != $t_updated_bug->description ) ||
								...
								$t_existing_bug->impact_analysis != $t_updated_bug->impact_analysis); // ADD HBT 
