...
<?php
//  HBT oriignal if( $f_error || $f_cookie_error || $f_reauthenticate){
	/* HBT modifiacion */ if( ($f_error || $f_cookie_error || $f_reauthenticate) && ($t_login_count==0 && $t_acceso!=2)) {
...
<?php if ( $t_login_count==0 && $t_acceso==2 ) { 
	$url ='HBT_password_change.php?username='.$t_username;
	header("Location: ".$url);
	exit();
	} else { ?> 
		<!-- FIN block code HBT -->
