...
require_api('email_api.php'); // ADD HBT
...
$f_autentication    = gpc_get_string( 'autentication' );// ADD HBT

//// ADD HBT
if($f_autentication == "mantis")
{
	$f_access = 2;
}else{
	if($f_autentication =="ldap"){
		$f_access = 1;
	}
} //// ADD HBT
...
if( !is_blank( $f_email ) && $f_access==2 ) {
	$t_confirm_hash = auth_generate_confirm_hash( $t_user_id );
	
	//$resultado = email_signup( $t_user_id, $f_password, $t_confirm_hash, '' ); HBT comentado mandaba dos veces el correo de nuevo usuario el correo se manda cuando se crea el usuario
